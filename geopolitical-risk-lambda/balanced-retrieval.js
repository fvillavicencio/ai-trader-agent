...
const { validateUrl } = require('./utils/validate-urls');
...
// RSS Feeds - All sources restored after confirming they work in our test function
const RSS_FEEDS = [
...
async function fetchRSSFeeds() {
  ...
  for (const feed of RSS_FEEDS) {
    try {
      logger.info(`Fetching RSS feed: ${feed.name} from ${feed.url}`);
      const startTime = Date.now();
      
      // Use Promise.race with explicit timeout to avoid hanging
      ...
      if (!feedResult.items || !Array.isArray(feedResult.items)) {
        logger.warn(`No items found in feed ${feed.name} or items is not an array`);
        stats.failedFeeds++;
        continue;
      }
      
      logger.info(`Found ${feedResult.items.length} items in ${feed.name}`);
      stats.totalItems += feedResult.items.length;
      ...
      for (const item of feedResult.items) {
        try {
          ...
          // Skip items outside time limit
          const pubDate = item.pubDate || item.isoDate;
          if (!isWithinTimeLimit(pubDate)) {
            continue;
          }
          ...
          if (!isGeopoliticallyRelevant(title + ' ' + content + ' ' + description)) {
            continue;
          }
          ...
          logger.warn(`Error processing item in feed ${feed.name}: ${itemError.message}`);
        }
      }
    } catch (feedError) {
      logger.error(`Failed to fetch RSS feed from ${feed.name}: ${feedError.message}`);
      logger.error(`Error stack: ${feedError.stack}`);
      console.error(`ERROR: Failed to fetch RSS feed from ${feed.name}: ${feedError.message}`);
      console.error(`ERROR: Error stack: ${feedError.stack}`);
      stats.failedFeeds++;
    }
  }
...
/**
 * Enhance geopolitical risks with OpenAI analysis
 * @param {Array} risks - Array of geopolitical risk objects
 * @returns {Promise<Array>} - Array of enhanced geopolitical risk objects
 */
async function enhanceRisksWithOpenAI(risks) {
  logger.info(`Enhancing ${risks.length} risks with OpenAI analysis`);
  const enhancedRisks = [];
  
  for (let i = 0; i < risks.length; i++) {
    const risk = risks[i];
    try {
      logger.info(`Enhancing risk ${i + 1}/${risks.length}: ${risk.name}`);
      ...
      const context = `
        ...
        Source: ${risk.source}
        Published Date: ${risk.publishedDate}
      `;
      ...
      const completion = await openai.chat.completions.create({
        ...
        messages: [
          {
            role: "system",
            content: "You are a geopolitical analyst specializing in identifying geopolitical risks and their potential impacts on global markets and supply chains. Provide concise, factual analysis."
          },
          {
            role: "user",
            content: `Analyze this geopolitical risk event and provide: 1) A concise summary (2-3 sentences), 2) Potential impact on global markets (1-2 sentences), 3) Regions most affected (comma-separated list), and 4) Related categories (comma-separated list from: trade, security, political, economic, environmental, technological). Here's the event: ${context}`
          }
        ],
        max_tokens: 300,
        temperature: 0.3
      ...
      const impactMatch = analysis.match(/impact:?\s*([^\n]+)/i) || analysis.match(/impact on global markets:?\s*([^\n]+)/i);
      const regionsMatch = analysis.match(/regions:?\s*([^\n]+)/i) || analysis.match(/regions most affected:?\s*([^\n]+)/i);
      const categoriesMatch = analysis.match(/categories:?\s*([^\n]+)/i) || analysis.match(/related categories:?\s*([^\n]+)/i);
      
      // Extract the matched content or use defaults
      const summary = summaryMatch ? summaryMatch[1].trim() : "";
      const impact = impactMatch ? impactMatch[1].trim() : "";
      const regions = regionsMatch ? regionsMatch[1].split(',').map(r => r.trim()) : ["Global"];
      const categories = categoriesMatch ? 
        categoriesMatch[1].split(',').map(c => c.trim().toLowerCase()) : 
        ["geopolitical"];
      
      // Add OpenAI analysis to the risk
      const enhancedRisk = {
        ...risk,
        aiSummary: summary,
        marketImpact: impact,
        regions: regions,
        categories: [...new Set([...risk.categories, ...categories])]
      };
      
      enhancedRisks.push(enhancedRisk);
      
      // Add a small delay to avoid rate limiting
      await new Promise(resolve => setTimeout(resolve, 500));
      
    } catch (error) {
      logger.error(`Error enhancing risk ${risk.name}: ${error.message}`);
      // If enhancement fails, add the original risk
      enhancedRisks.push(risk);
    }
  }
  
  logger.info(`Successfully enhanced ${enhancedRisks.length} risks with OpenAI analysis`);
...
async function retrieveGeopoliticalRisks() {
  try {
    ...
    // Enhance risks with OpenAI analysis
    logger.info('Enhancing geopolitical risks with OpenAI analysis...');
    const enhancedRisks = await enhanceRisksWithOpenAI(topRisks);
    
    // Save risks to file
    await saveRisks(enhancedRisks);
    
    // Log statistics
    logger.info(`Retrieved and enhanced ${enhancedRisks.length} geopolitical risks`);
...
module.exports = {
  retrieveGeopoliticalRisks,
  enhanceRisksWithOpenAI,
  fetchRSSFeeds,
  fetchGoogleNewsRSS,
  fetchNewsAPI,
  fetchRapidAPI,
  isGeopoliticallyRelevant,
  fetchZeihanContent,
  deduplicateEvents
};
