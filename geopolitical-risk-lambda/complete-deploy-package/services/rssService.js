...
const RSS_FEEDS = [
  ...
  // Updated Brookings feed URL
  'https://www.brookings.edu/feed',
  // Updated CSIS feed URL
...
async function fetchRSSFeed(url) {
  try {
    // Use axios with a shorter timeout (15 seconds instead of default 60)
    ...
    logger.info(`Successfully fetched RSS feed from ${url}`);
    return feed.items || [];
  } catch (error) {
    // More detailed error logging
    if (error.response) {
      logger.error(`Error fetching RSS feed ${url}: Status code ${error.response.status}`);
    } else if (error.request) {
      logger.error(`Error fetching RSS feed ${url}: ${error.message || 'No response received'}`);
    } else {
