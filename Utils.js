                        ...
                        <tbody>
                          ${Object.keys(metricLabels).map(key => {
                            if (stock[key] === undefined || stock[key] === null || stock[key] === '') return '';
                             ...
                             if (key === 'marketCap') displayValue = formatMarketCap(stock[key]);
                             return `<tr><td style=\"color: #777; padding: 2px 8px 2px 0;\">${metricLabels[key]}</td><td style=\"font-weight: bold; color: #222; padding: 2px 0; text-align: right;\">${formatStockMetricValue(key, displayValue)}</td></tr>`;
                          }).join('')}
                        </tbody>
                      </table>
                    </div>
