...
/**
 * Test function to call the Geopolitical Risk Google Cloud Function.
 * This function retrieves the API key from Script Properties (named 'GCF_API_KEY').
 * It then calls the GCF endpoint and logs the response.
 */
function testGeopoliticalRiskGCF() {
  Logger.log('Starting Geopolitical Risk Google Cloud Function Test...');

  const gcfUrl = 'https://us-central1-geopolitical-risk-analysis.cloudfunctions.net/geopoliticalRiskAPI'; // As per MEMORY[7c09e232-42ef-4986-ba93-5e632c158544]
  const apiKey = PropertiesService.getScriptProperties().getProperty('GEOPOLITICAL_RISKS_API_KEY');

  if (!apiKey) {
    Logger.log('ERROR: API Key for GCF not found in Script Properties. Please set GEOPOLITICAL_RISKS_API_KEY.');
    console.error('ERROR: API Key for GCF not found in Script Properties. Please set GEOPOLITICAL_RISKS_API_KEY.');
    return;
  }
  ...
  const options = {
    ...
    'contentType': 'application/json',
    'muteHttpExceptions': true // Set to true to get full error response, false to throw exception on HTTP error codes
  };

  try {
    Logger.log(`Calling GCF endpoint: ${gcfUrl}`);
    ...
    if (responseCode === 200) {
      Logger.log('GCF call successful.');
      Logger.log('Response Body (first 500 chars): ' + responseBody.substring(0, 500));
      // You can parse the JSON if needed:
      // const jsonResponse = JSON.parse(responseBody);
      // Logger.log(JSON.stringify(jsonResponse, null, 2));
    } else {
      Logger.log(`Error calling GCF. Response Code: ${responseCode}`);
      Logger.log(`Error Response Body: ${responseBody}`);
      console.error(`Error calling GCF. Response Code: ${responseCode}. Body: ${responseBody}`);
    }
    
    // To return the full response for further inspection if running from Apps Script editor
    // return responseBody; 
    
  } catch (e) {
    Logger.log(`Exception during GCF call: ${e.toString()}`);
    console.error(`Exception during GCF call: ${e.toString()}`);
