 ...
 * @param {Date|String|Object} date - Date to format (can be Date object, string, or object with date properties)
 * @return {String} Formatted date
 */
function formatDate(date) {
  // Safety check - if null or undefined, return 'N/A'
  if (date === null || date === undefined) {
    return 'N/A';
  }
  
  // If it's already a string in MM/DD/YYYY format, return it as is
  ...
  // If it's an object with date components (like a timestamp object)
  if (typeof date === 'object') {
    // Try to extract date components if they exist
    if (date.year && (date.month || date.getMonth) && (date.day || date.date || date.getDate)) {
      const year = date.year || date.getFullYear();
      const month = date.month || date.getMonth() + 1;
      const day = date.day || date.date || date.getDate();
