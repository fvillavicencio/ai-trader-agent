...
function formatMetricValue(key, value) {
  ...
  switch(key) {
    ...
    // Fix percentage values that are in decimal form (0-1 range)
    case 'divYield':
    case 'dividendYield':
      // If it's already in percentage form (>1), display as is, otherwise multiply by 100
      return (numValue < 1 && numValue > 0) ? (numValue * 100).toFixed(2) : numValue.toFixed(2);
    case 'profitMargin':
    case 'returnOnEquity':
    case 'returnOnAssets':
    case 'roe':
    case 'roa':
      // If it's already in decimal form (<1), multiply by 100 to get proper percentage
      return (numValue < 1 && numValue > 0) ? (numValue * 100).toFixed(2) : numValue.toFixed(2);
    default:
      return value;
  }
