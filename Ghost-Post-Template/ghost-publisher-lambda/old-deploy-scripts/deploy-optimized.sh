...
# Ghost Publisher Lambda - Optimized Deployment Script
...
// We need to handle aws-sdk differently
// The main aws-sdk is provided by Lambda, but we need to keep specific clients that are directly imported
// For example, if code imports 'aws-sdk/clients/s3' directly, we need to keep that path

// Instead of excluding the entire aws-sdk package, we'll include it but modify our approach
EXCLUDE_PACKAGES=\"aws-sdk/clients/*\"
// Note: We're keeping aws-sdk/clients/* paths since they're directly imported in the code
const dependencies = {};
for (const [key, value] of Object.entries(pkg.dependencies)) {
  if (!key.startsWith('aws-sdk/clients/')) {
    dependencies[key] = value;
  }
}
minPkg.dependencies = dependencies;
