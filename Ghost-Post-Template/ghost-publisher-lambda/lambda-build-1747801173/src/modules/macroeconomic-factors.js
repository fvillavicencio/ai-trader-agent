...
const addUpcomingEvents = (data) => {
  // Get economic events from the data object
  const dataEvents = data.marketIndicators?.economicEvents || [];
  
  // Use events from the data if available, otherwise use empty array
  const upcomingEvents = dataEvents.length > 0 ? dataEvents : [];
  
  // If no events are available, return empty HTML
          ...
          ${sortedEvents.map(event => `
            <div style="display: flex; flex-wrap: nowrap; align-items: flex-start; padding: 8px 10px; border-radius: 5px; background-color: #ffffff; border: 1px solid #e2e8f0;">
              ...
              <div style="flex: 1 1 120px; min-width: 120px; max-width: 100%; text-align: right; color: #555; font-size: 0.85rem;">
                <div style="margin-bottom: 2px;"><span style="color: #2563eb; font-weight: 500;">Forecast:</span> ${event.forecast || 'N/A'}</div>
                <div><span style="color: #64748b; font-weight: 500;">Previous:</span> ${event.previous || 'N/A'}</div>
              </div>
            </div>
          `).join('')}
        </div>
