...
/**
 * Add financial image to the HTML content with a sentiment-appropriate quote
 * @param {string} htmlContent - The HTML content to modify
 * @param {string} imageUrl - The URL of the image to add
 * @param {string} sentiment - The market sentiment (bullish, bearish, neutral, volatile)
 * @returns {string} - The modified HTML content with the image and quote
 */
const addFinancialImage = (htmlContent, imageUrl, sentiment = 'neutral') => {
    // Select a quote based on the sentiment
    let quote = '';
    let author = '';
    
    // Map of quotes by sentiment
    const sentimentQuotes = {
        'bullish': [
            { quote: "Greed, for lack of a better word, is good.", author: "Gordon Gekko" },
        ...
        'bearish': [
            { quote: "I don't think most people want to accept that they're in a bubble.", author: "Michael Burry" },
            ...
            { quote: "It's only when the tide goes out that you learn who's been swimming naked.", author: "Warren Buffett" }
        ],
        'neutral': [
            ...
            { quote: "Tug of war between optimism and pessimism is what makes markets function.", author: "Ray Dalio" }
        ],
        'volatile': [
            { quote: "The market is a pendulum that forever swings between unsustainable optimism and unjustified pessimism.", author: "Howard Marks" },
            { quote: "Volatility is not the same as risk, and anyone who thinks it is will cost themselves money.", author: "Warren Buffett" },
    ...
    // Get quotes for the specified sentiment, or default to neutral
    const quotes = sentimentQuotes[sentiment] || sentimentQuotes['neutral'];
    
    // Select a quote based on the image URL to ensure consistency
    // Extract a value from the image URL to use as a seed
    const urlSeed = imageUrl.split('').reduce((acc, char) => acc + char.charCodeAt(0), 0);
    const selectedQuote = quotes[urlSeed % quotes.length];
    
    quote = selectedQuote.quote;
    author = selectedQuote.author;
    
    // Create the image HTML with the selected quote
    const imageHtml = `
    <div style="text-align: center; margin: 20px 0 30px 0;">
      ...
      <p style="font-style: italic; color: #718096; margin-top: 10px; font-size: 0.9rem;">"${quote}" - ${author}</p>
    </div>
    `;
...
exports.handler = async (event, context) => {
    try {
        ...
        // Fix S&P 500 display format in the mobiledoc
        if (mobiledoc && mobiledoc.cards) {
            for (let i = 0; i < mobiledoc.cards.length; i++) {
                const card = mobiledoc.cards[i];
                if (card[0] === 'html' && card[1].html && card[1].html.includes('S&P 500:')) {
                    // Fix the S&P 500 display format directly in the card HTML
                    card[1].html = card[1].html.replace(/S&P 500: ([\d,]+) <span style="color: (#[0-9a-f]+)">\(([↑↓]) ([\d.]+)[^<]*<\/span>/g, 
                                                'S&P 500: $1 <span style="color: $2">$3 $4</span>');
                    console.log('Fixed S&P 500 display format in mobiledoc card');
                }
            }
        }
        ...
        // For "Sell Calls" decision, always use bearish sentiment
        let sentiment = 'neutral';
        if (data.decision) {
            if (data.decision.sentiment) {
                sentiment = data.decision.sentiment;
            } else if (data.decision.action && data.decision.action.toLowerCase().includes('sell call')) {
                sentiment = 'bearish';
            }
        }
        console.log(`Using sentiment "${sentiment}" for image selection`);
        
        // Add a timestamp to ensure we get a different image each time
        // This forces the random selection to be truly random for each post
        const currentTime = new Date().getTime();
        const seed = currentTime.toString();
        const randomizedTitle = `${title} ${currentTime}`;
        console.log(`Using randomized title for image selection: ${randomizedTitle}`);
        console.log(`Using seed for image selection: ${seed}`);
        ...
        const s3ImageResult = getS3ImageForTitle(randomizedTitle, sentiment, seed);
        ...
        // For "Sell Calls" decision, always use the Michael Burry image
        let selectedImageUrl;
        
        // For "Sell Calls" decision, we want to specifically use the Michael Burry image
        if (data.decision && data.decision.action && data.decision.action.toLowerCase().includes('sell call')) {
            selectedImageUrl = 'https://market-pulse-daily-title-images.s3.us-east-2.amazonaws.com/bearish/the_big_short/michael_burry_big_short.jpg';
            console.log(`Using Michael Burry image for Sell Calls decision`);
        } else {
            // Map of sentiment to specific image folders that actually exist in the S3 bucket
            const sentimentImageMap = {
                'bullish': [
                    'bullish/absolutely_vertical/absolutely_vertical_skyscraper_stock_chart_upward_1.jpg',
                    'bullish/absolutely_vertical/absolutely_vertical_skyscraper_stock_chart_upward_2.jpg',
                    'bullish/absolutely_vertical/absolutely_vertical_stock_market_surge_5.jpg',
                    'bullish/absolutely_vertical/absolutely_vertical_stock_market_surge_6.jpg',
                    'bullish/absolutely_vertical/jordan_belfort_wolf_of_wall_street.jpg'
                ],
                'bearish': [
                    'bearish/the_correction_is_coming/the_correction_is_coming_dark_clouds_wall_street_3.jpg',
                    'bearish/the_correction_is_coming/the_correction_is_coming_dark_clouds_wall_street_4.jpg',
                    'bearish/the_correction_is_coming/the_correction_is_coming_storm_clouds_financial_district_1.jpg',
                    'bearish/the_correction_is_coming/the_correction_is_coming_storm_clouds_financial_district_2.jpg',
                    'bearish/the_big_short/michael_burry_big_short.jpg'
                ],
                'neutral': [
                    'neutral/mixed_signals/mixed_signals_market_crossroads_1.jpg',
                    'neutral/mixed_signals/mixed_signals_market_crossroads_2.jpg',
                    'neutral/the_waiting_game/the_waiting_game_market_analysis_1.jpg',
                    'neutral/the_waiting_game/the_waiting_game_market_analysis_2.jpg',
                    'neutral/the_crossroads/the_crossroads_market_decision_1.jpg'
                ],
                'volatile': [
                    'volatile/market_whiplash/market_whiplash_volatility_chart_1.jpg',
                    'volatile/market_whiplash/market_whiplash_volatility_chart_2.jpg',
                    'volatile/the_perfect_storm/the_perfect_storm_market_turbulence_1.jpg',
                    'volatile/the_perfect_storm/the_perfect_storm_market_turbulence_2.jpg',
                    'volatile/buckle_up/buckle_up_market_rollercoaster_1.jpg'
                ]
            };
            
            // Use the title and timestamp to select a different image for each post
            if (sentimentImageMap[sentiment]) {
                // Create a more unique seed by combining title and timestamp
                // This ensures different posts with the same sentiment get different images
                const titleHash = title.split('').reduce((acc, char) => acc + char.charCodeAt(0), 0);
                
                // Use a more complex algorithm to ensure better diversity
                // We'll use the first character of the title to add additional randomness
                const firstCharCode = title.charCodeAt(0) || 65;
                const secondCharCode = title.length > 1 ? title.charCodeAt(1) : 66;
                
                // Combine multiple factors for better distribution
                const uniqueIndex = Math.abs((titleHash * firstCharCode + secondCharCode + currentTime)) % sentimentImageMap[sentiment].length;
                
                const imagePath = sentimentImageMap[sentiment][uniqueIndex];
                selectedImageUrl = `https://market-pulse-daily-title-images.s3.us-east-2.amazonaws.com/${imagePath}`;
                console.log(`Directly selected image: ${imagePath} for title: ${title} (${sentiment})`);
            } else {
                // Fallback to default image
                console.log('No suitable image found, using default image');
                selectedImageUrl = 'https://market-pulse-daily-title-images.s3.us-east-2.amazonaws.com/bullish/absolutely_vertical/jordan_belfort_wolf_of_wall_street.jpg';
            }
        }
        
        const gekkoImageUrl = selectedImageUrl;
        ...
        // Ensure the feature image is properly set
        console.log(`Setting feature image URL: ${gekkoImageUrl}`);
        console.log('Post data prepared:', JSON.stringify({
            ...postData,
            mobiledoc: '(mobiledoc content omitted for brevity)'
        }));
        
        // Direct fix for S&P 500 display format in the mobiledoc before sending to Ghost
        console.log('Fixing S&P 500 display format in mobiledoc JSON before sending to Ghost...');
        const mobiledocStr = JSON.stringify(postData.mobiledoc);
        const fixedMobiledocStr = mobiledocStr.replace(/\(([\u2191\u2193]) ([\d.]+) \([\d.]+%\)\)/g, '$1 $2');
        postData.mobiledoc = JSON.parse(fixedMobiledocStr);
        console.log('Fixed S&P 500 display format in mobiledoc JSON');
        ...
        // Add financial image to the HTML content with a quote matching the sentiment
        console.log('Adding financial image to HTML content...');
        html = addFinancialImage(html, gekkoImageUrl, sentiment);
        
        // Clean up S&P 500 display format in the HTML (remove parentheses and percentage)
        console.log('Cleaning up S&P 500 display format in HTML...');
        
        // Use a regex pattern to find and modify the S&P 500 display format
        // This pattern matches the S&P 500 display with parentheses and percentage
        const sp500Pattern = /<span style="white-space: nowrap;">S&P 500: ([\d,]+) <span style="color: (#[0-9a-f]+)">\(([↑↓]) ([\d.]+)[^<]*<\/span><\/span>/g;
        
        // Replace with a cleaner format without parentheses and percentage
        html = html.replace(sp500Pattern, '<span style="white-space: nowrap;">S&P 500: $1 <span style="color: $2">$3 $4</span></span>');
        
        console.log('Completed S&P 500 display format cleanup');
