 ...
 */
function getImageForTitle(title, sentiment = 'neutral', seed = '') {
  try {
        ...
        if (fs.existsSync(folderPath)) {
          try {
            ...
            if (characterImages.length > 0 && Math.random() < 0.75) {
              // Use the seed to create a deterministic selection
              const seedValue = seed ? seed.toString().split('').reduce((a, b) => a + b.charCodeAt(0), 0) : Date.now();
              const imageIndex = seedValue % characterImages.length;
              
              // Select a character image based on the seed
              const selectedImage = characterImages[imageIndex];
    ...
    if (!folderPath && sentimentFolders[sentiment]) {
      ...
      // Use the seed for deterministic selection
      const seedValue = seed ? seed.toString().split('').reduce((a, b) => a + b.charCodeAt(0), 0) : Date.now();
      const folderIndex = seedValue % fallbackFolders.length;
      folderPath = fallbackFolders[folderIndex];
    }
    ...
    if (!fs.existsSync(fullFolderPath)) {
      ...
      if (fs.existsSync(sentimentDir)) {
        ...
        if (availableFolders.length > 0) {
          // Choose a folder based on the seed
          const seedValue = seed ? seed.toString().split('').reduce((a, b) => a + b.charCodeAt(0), 0) : Date.now();
          const folderIndex = seedValue % availableFolders.length;
          folderPath = availableFolders[folderIndex];
          console.log(`Using alternative folder: ${folderPath}`);
          return getImageFromFolder(folderPath, seed);
        }
      }
 ...
 */
function getImageFromFolder(folderPath, seed = '') {
  try {
    const fullFolderPath = path.join(BASE_IMAGE_DIR, folderPath);
    
    if (!fs.existsSync(fullFolderPath)) {
      console.warn(`Folder does not exist: ${fullFolderPath}`);
      return findAnyAvailableImage(seed);
    }
    ...
    // Use the seed to create a more random selection
    // This ensures different posts get different images
    const seedValue = seed ? seed.toString().split('').reduce((a, b) => a + b.charCodeAt(0), 0) : Date.now();
    const randomIndex = seedValue % images.length;
    
    // Select a random image
    const selectedImage = images[randomIndex];
    console.log(`Selected image ${randomIndex + 1} of ${images.length} from ${folderPath}: ${selectedImage}`);
 ...
 */
function findAnyAvailableImage(seed = '') {
  try {
    // Try to find an image in any sentiment category
    const sentiments = ['bullish', 'bearish', 'neutral', 'volatile'];
    
    // Use the seed to create a more random selection
    const seedValue = seed ? seed.toString().split('').reduce((a, b) => a + b.charCodeAt(0), 0) : Date.now();
    
    // Shuffle the sentiments based on the seed
    const shuffledSentiments = [...sentiments];
    for (let i = shuffledSentiments.length - 1; i > 0; i--) {
      const j = (seedValue + i) % (i + 1);
      [shuffledSentiments[i], shuffledSentiments[j]] = [shuffledSentiments[j], shuffledSentiments[i]];
    }
    
    for (const sentiment of shuffledSentiments) {
      const sentimentDir = path.join(BASE_IMAGE_DIR, sentiment);
      
      if (fs.existsSync(sentimentDir)) {
        // Get all folders in this sentiment category
        ...
        if (folders.length > 0) {
          // Choose a folder based on the seed
          const folderIndex = seedValue % folders.length;
          const selectedFolder = folders[folderIndex];
          const folderPath = path.join(sentiment, selectedFolder);
          ...
          if (images.length > 0) {
            // Choose an image based on the seed
            const imageIndex = (seedValue + folderIndex) % images.length;
            const selectedImage = images[imageIndex];
            console.log(`Selected fallback image ${imageIndex + 1} of ${images.length} from ${folderPath}: ${selectedImage}`);
            return path.join(folderPath, selectedImage);
          }
        }
      }
    ...
    console.error('No images found in any category');
    return null;
  } catch (error) {
