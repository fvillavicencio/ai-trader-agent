/**
 * Test script for the Ghost Publisher Lambda function with pure null RSI value
 * This script tests our enhanced fix for handling null RSI values
 */
...
// Load the JSON data
const data = JSON.parse(fs.readFileSync(path.resolve(__dirname, 'test-rsi-pure-null.json'), 'utf8'));

console.log('Testing Lambda function with pure null RSI value...');
...
async function runTest() {
  try {
    console.log('Starting Lambda test with pure null RSI value...');
    ...
    module.exports = {
      handler: async (event, context) => {
        ...
        try {
          ...
          fs.writeFileSync('rsi-pure-null-response.json', JSON.stringify(result, null, 2));
          console.log('Response saved to rsi-pure-null-response.json');
          ...
          if (result && result.body) {
            ...
            if (body && body.url) {
              ...
              const report = `
RSI Test Report - Pure Null Value
================================
Test Date: ${new Date().toISOString()}
Test Type: RSI with pure null value
...
`;
              fs.writeFileSync('rsi-pure-null-report.txt', report);
              console.log('Test report saved to rsi-pure-null-report.txt');
            }
          }
        } catch (error) {
