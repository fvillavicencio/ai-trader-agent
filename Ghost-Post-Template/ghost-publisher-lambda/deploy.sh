...
# Ghost Publisher Lambda - Clean Deployment Script
...
find node_modules -type d -name "test" -o -name "tests" -o -name ".github" -o -name "docs" | xargs rm -rf 2>/dev/null || true
find node_modules -type f -name "*.md" -o -name "LICENSE*" -o -name "*.ts" -o -name "*.map" | xargs rm -f 2>/dev/null || true

# Create the deployment package - make sure to create a fresh zip file
...
if [ ! -f "function.zip" ]; then
  echo "❌ Error: function.zip file not found!"
  exit 1
fi
