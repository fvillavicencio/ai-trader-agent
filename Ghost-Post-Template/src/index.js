...
function formatMetricValue(key, value) {
  ...
  // Convert to number if it's a string but represents a number
  ...
  switch(key) {
    // P/E ratios and similar metrics should be displayed as multiples with 'x'
    case 'pe':
    case 'forwardPE':
    case 'priceToEarnings':
      ...
      return typeof value === 'number' ? Number(value).toFixed(1) + 'x' : value;
    // PEG ratio is a special case - it's a ratio but not typically shown with 'x'
    case 'pegRatio':
    ...
    case 'roa':
      ...
      return typeof value === 'number' ? Number(value).toFixed(2) + '%' : value;
    // Beta and other decimal metrics
    case 'beta':
      return typeof value === 'number' ? Number(value).toFixed(2) : value;
    default:
      return value;
