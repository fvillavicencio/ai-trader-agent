 ...
 * 
 * This script extracts Fear and Greed Index data from Ghost blog posts
...
program
  .option('--key <key>', 'Ghost Admin API key')
  .option('--content-key <key>', 'Ghost Content API key')
  .option('--url <url>', 'Ghost URL')
  .option('--use-content-api', 'Force use of Content API even if Admin API key is available')
  .parse();
...
const useContentApi = options.useContentApi || false;
...
// Initialize Admin API if key is available and we're not forced to use Content API
if (ghostApiKey && !useContentApi) {
  try {
...
// Initialize Content API if key is available
if (contentApiKey) {
  try {
