...
function generateMarketIndicatorsSection(analysis) {
  try {
    ...
    if (indicators.majorIndices && Array.isArray(indicators.majorIndices)) {
      ...
      <div style="margin-bottom: 15px;">
        ...
        <div style="display: flex; background-color: #f8f9fa; padding: 15px; border-radius: 6px; margin-bottom: 10px;">
          ${indicators.majorIndices.map(index => `
            <div style="flex: 1; text-align: center; padding: 0 10px; position: relative;">
              <div style="color: #666; font-size: 14px; margin-bottom: 8px;">${index.name}</div>
              <div style="color: #4CAF50; font-weight: bold; font-size: 20px;">${formatValue(index.price)}%</div>
        ...
        <div style="margin-top: 15px; padding-left: 15px; border-left: 4px solid #FFA500;">
          <div style="font-weight: bold; margin-bottom: 5px;">Yield Curve: ${indicators.majorIndices[0]?.yieldCurve?.status || 'N/A'}</div>
          <div style="color: #555; font-size: 14px;">${indicators.majorIndices[0]?.yieldCurve?.analysis || 'N/A'}</div>
        </div>

        <div style="font-size: 12px; color: #888; margin-top: 10px; text-align: right;">
          Source: ${indicators.majorIndices[0]?.source || 'N/A'} (${indicators.majorIndices[0]?.sourceUrl || 'N/A'}), as of ${formatDate(indicators.majorIndices[0]?.timestamp)}
        </div>
      </div>
      `;
    }
    ...
    if (indicators.sectorPerformance && Array.isArray(indicators.sectorPerformance)) {
      sectorsHtml = `
      <div style="margin-bottom: 15px;">
        <div style="font-weight: bold; margin-bottom: 10px;">Sector Performance</div>
        <div style="display: flex; background-color: #f8f9fa; padding: 15px; border-radius: 6px; margin-bottom: 10px;">
          ${indicators.sectorPerformance.map(sector => `
            <div style="flex: 1; text-align: center; padding: 0 10px; position: relative;">
              <div style="color: #666; font-size: 14px; margin-bottom: 8px;">${sector.name}</div>
              <div style="color: #4CAF50; font-weight: bold; font-size: 20px;">${formatValue(sector.percentChange)}%</div>
              <div style="position: absolute; top: 0; bottom: 0; left: 0; width: 3px; background-color: #4CAF50;"></div>
            </div>
          `).join('')}
        </div>
        <div style="font-size: 12px; color: #888; margin-top: 10px; text-align: right;">
          Source: ${indicators.sectorPerformance[0]?.source || 'N/A'} (${indicators.sectorPerformance[0]?.sourceUrl || 'N/A'}), as of ${formatDate(indicators.sectorPerformance[0]?.timestamp)}
        </div>
      </div>
      `;
    }
    ...
    if (indicators.volatilityIndices && Array.isArray(indicators.volatilityIndices)) {
      vixHtml = `
      <div style="margin-bottom: 15px;">
        <div style="font-weight: bold; margin-bottom: 10px;">Volatility Indices</div>
        <div style="display: flex; background-color: #f8f9fa; padding: 15px; border-radius: 6px; margin-bottom: 10px;">
          ${indicators.volatilityIndices.map(vix => `
            <div style="flex: 1; text-align: center; padding: 0 10px; position: relative;">
              <div style="color: #666; font-size: 14px; margin-bottom: 8px;">${vix.name}</div>
              <div style="color: ${vix.value >= 30 ? '#f44336' : vix.value >= 20 ? '#ff9800' : '#4caf50'}; font-weight: bold; font-size: 20px;">${formatValue(vix.value)}%</div>
              <div style="position: absolute; top: 0; bottom: 0; left: 0; width: 3px; background-color: ${vix.value >= 30 ? '#f44336' : vix.value >= 20 ? '#ff9800' : '#4caf50'};"></div>
            </div>
          `).join('')}
        </div>
        <div style="font-size: 12px; color: #888; margin-top: 10px; text-align: right;">
    ...
    if (indicators.upcomingEconomicEvents && Array.isArray(indicators.upcomingEconomicEvents) && indicators.upcomingEconomicEvents.length > 0) {
      ...
      eventsHtml = `
      <div style="margin-bottom: 15px;">
        <div style="font-weight: bold; margin-bottom: 10px;">Upcoming Events</div>
        <div style="display: flex; background-color: #f8f9fa; padding: 15px; border-radius: 6px; margin-bottom: 10px;">
          ${sortedEvents.map(event => `
            <div style="flex: 1; text-align: center; padding: 0 10px; position: relative;">
              <div style="color: #666; font-size: 14px; margin-bottom: 8px;">${event.date}</div>
              <div style="color: #333; font-weight: bold; font-size: 20px;">${event.event}</div>
              <div style="position: absolute; top: 0; bottom: 0; left: 0; width: 3px; background-color: #333;"></div>
            </div>
          `).join('')}
        </div>
        <div style="font-size: 12px; color: #888; margin-top: 10px; text-align: right;">
          Source: ${indicators.upcomingEconomicEvents[0]?.source || 'N/A'} (${indicators.upcomingEconomicEvents[0]?.sourceUrl || 'N/A'}), as of ${formatDate(indicators.upcomingEconomicEvents[0]?.timestamp)}
        </div>
      </div>
      `;
    }
            ...
            <div style="margin-bottom: 10px;">
              ...
              <div style="display: flex; justify-content: space-between; align-items: center;">
                <div style="color: #f44336; font-size: 14px;">
                  <span style="font-size: 1.5em;">↑</span> <span style="font-size: 2.5em;">${macro.fedPolicy.nextMeeting.probabilityOfHike}%</span>
                </div>
                <div style="color: #757575; font-size: 14px;">
                  <span style="font-size: 1.5em;">→</span> <span style="font-size: 2.5em;">${macro.fedPolicy.nextMeeting.probabilityOfNoChange}%</span>
                </div>
                <div style="color: #4CAF50; font-size: 14px;">
                  <span style="font-size: 1.5em;">↓</span> <span style="font-size: 2.5em;">${macro.fedPolicy.nextMeeting.probabilityOfCut}%</span>
                </div>
              </div>
            </div>
          </div>
