...
const OPENAI_API_KEY = ""; // Don't hardcode the key here, use Script Properties instead
const OPENAI_MODEL = "gpt-4-turbo-preview"; // Using GPT-4 Turbo with retrieval
...
const DATA_CACHE = {
  ...
  storeTreasuryYields: function(yields) {
    ...
    Logger.log("Treasury yields data cached successfully.");
  },
  ...
  getTreasuryYields: function() {
    ...
    Logger.log("No valid treasury yields data in cache or cache expired");
    return null;
  },
  ...
  isTreasuryYieldsCacheValid: function() {
    const currentTime = new Date().getTime();
    const isValid = this.treasuryYields && this.lastTreasuryYieldsFetch && 
           (currentTime - this.lastTreasuryYieldsFetch < this.treasuryYieldsCacheExpiry);
    
    if (!isValid && this.lastTreasuryYieldsFetch) {
      const cacheAge = (currentTime - this.lastTreasuryYieldsFetch) / (1000 * 60 * 60);
      Logger.log(`Treasury yields cache invalid. Cache age: ${cacheAge.toFixed(2)} hours`);
    }
...
/**
 * Gets the OpenAI API key from script properties if not hardcoded
 * For better security, use PropertiesService instead of hardcoding the API key
 * 
...
/**
 * Gets the BLS (Bureau of Labor Statistics) API key from script properties
 * For better security, use PropertiesService instead of hardcoding the API key
 * 
...
/**
 * Gets the BEA (Bureau of Economic Analysis) API key from script properties
 * For better security, use PropertiesService instead of hardcoding the API key
 * 
...
/**
 * Gets the FRED (Federal Reserve Economic Data) API key from script properties
 * For better security, use PropertiesService instead of hardcoding the API key
 * 
...
function getYahooFinanceApiKey() {
  try {
    ...
    Logger.log("Yahoo Finance API key not found in script properties");
    return null;
  } catch (error) {
