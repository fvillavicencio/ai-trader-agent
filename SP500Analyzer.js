...
function formatSP500AnalysisText(lambdaJson) {
  ...
  if (data.forwardEstimates && data.forwardEstimates.length > 0) {
    ...
    data.forwardEstimates.forEach(function(est) {
      lines.push(`  ${est.scenario} | ${est.year} | ${formatDate_(est.estimateDate)} | $${est.eps} | ${est.pe15} | ${est.pe15Pct} | ${est.pe17} | ${est.pe17Pct} | ${est.pe20} | ${est.pe20Pct} | ${est.sourceUrl || ''}`);
    });
    lines.push('');
  }
  ...
  if (data.earnings) {
    ...
    lines.push(`  EPS (TTM) | ${data.earnings.eps || 'N/A'}`);
    lines.push(`  S&P 500 Target at 15x | ${data.earnings.pe15 || 'N/A'}`);
    lines.push(`  S&P 500 Target at 17x | ${data.earnings.pe17 || 'N/A'}`);
    lines.push(`  S&P 500 Target at 20x | ${data.earnings.pe20 || 'N/A'}`);
...
function formatSP500AnalysisHtml(lambdaJson) {
  ...
  if (data.forwardEstimates && data.forwardEstimates.length > 0) {
    ...
    html.push('<div class="forward-pe-block"><div class="responsive-table"><table><thead><tr><th>Scenario</th><th>Year</th><th>Estimate Date</th><th>Forward EPS</th><th>15x</th><th>% vs Index</th><th>17x</th><th>% vs Index</th><th>20x</th><th>% vs Index</th><th>Source URL</th></tr></thead><tbody>');
    data.forwardEstimates.forEach(function(est) {
      html.push(`<tr><td>${est.scenario}</td><td>${est.year}</td><td>${formatDate_(est.estimateDate)}</td><td><strong>$${est.eps}</strong></td><td>${est.pe15}</td><td>${est.pe15Pct}</td><td>${est.pe17}</td><td>${est.pe17Pct}</td><td>${est.pe20}</td><td>${est.pe20Pct}</td><td><a href="${est.sourceUrl || '#'}">link</a></td></tr>`);
    });
  ...
  if (data.earnings) {
    ...
    html.push(`<tr><td><strong>EPS (TTM)</strong></td><td><strong>${data.earnings.eps || 'N/A'}</strong></td></tr>`);
    html.push(`<tr><td>S&P 500 Target at <strong>15x</strong></td><td><strong>${data.earnings.pe15 || 'N/A'}</strong></td></tr>`);
    html.push(`<tr><td>S&P 500 Target at <strong>17x</strong></td><td><strong>${data.earnings.pe17 || 'N/A'}</strong></td></tr>`);
    html.push(`<tr><td>S&P 500 Target at <strong>20x</strong></td><td><strong>${data.earnings.pe20 || 'N/A'}</strong></td></tr>`);
    html.push('</tbody></table></div>');
    html.push(`<div class="source-block"><strong>Source:</strong> ${data.earnings.sourceName}<br/><strong>URL:</strong> <a href="${data.earnings.sourceUrl || '#'}">${data.earnings.sourceUrl || ''}</a><br/><strong>Last Update:</strong> ${formatDate_(data.earnings.lastUpdated)}</div>`);
    html.push('</div>');
  }
