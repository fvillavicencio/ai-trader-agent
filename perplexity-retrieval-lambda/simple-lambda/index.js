...
exports.handler = async (event = {}) => {
  ...
  try {
    ...
    // Explicitly convert to boolean to avoid any string comparison issues
    const envFallback = process.env.USE_FALLBACK ? process.env.USE_FALLBACK.toLowerCase() === 'true' : false;
    const eventFallback = event.useFallback === true;
    const useFallback = envFallback || eventFallback;
    console.log('USE_FALLBACK:', useFallback, '(env:', envFallback, ', event:', eventFallback, ')');

    // If fallback is requested, return fallback data
    if (useFallback) {
      console.log('Using fallback data due to USE_FALLBACK flag');
      const fallbackData = createFallbackData();
      console.log(`Generated fallback data with ${fallbackData.risks.length} risks and index ${fallbackData.geopoliticalRiskIndex}`);
    ...
    console.log('Error encountered, using fallback data:', error.message);
