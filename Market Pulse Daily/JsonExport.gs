...
function generateFullJsonDataset(analysisJson, debugMode = false) {
  try {
    ...
    if (fullJsonDataset.sp500 && !fullJsonDataset.sp500.forwardPE && fullJsonDataset.sp500.forwardEps && fullJsonDataset.sp500.indexLevel) {
      ...
      let epsValue = currentForwardEps.eps;
      ...
      if (!isNaN(epsValue) && epsValue > 0) {
        ...
        const forwardPE = fullJsonDataset.sp500.indexLevel / epsValue;
        
        // Add the forward P/E data to the SP500 object
        fullJsonDataset.sp500.forwardPE = {
          current: parseFloat(forwardPE.toFixed(2)),
          eps: currentForwardEps.eps,
          year: currentForwardEps.year,
          source: fullJsonDataset.sp500.forwardEpsSource ? fullJsonDataset.sp500.forwardEpsSource.name : "S&P Global",
          sourceUrl: fullJsonDataset.sp500.forwardEpsSource ? fullJsonDataset.sp500.forwardEpsSource.url : "https://www.spglobal.com/spdji/en/",
          lastUpdated: fullJsonDataset.sp500.forwardEpsSource ? fullJsonDataset.sp500.forwardEpsSource.asOf : formattedDate
        };
        
        Logger.log(`Added forward P/E ratio: ${forwardPE.toFixed(2)} based on EPS: ${epsValue} and index level: ${fullJsonDataset.sp500.indexLevel}`);
      }
    }
